...
@4 /* Nice to see some faces that look like they've a hankering for some action! Just give me a moment to get down from here... */
...
@5 /* Ha ha, did you like my little display of acrobatics? I apologize if my presence on the roof shocked you. */
DO ~SetGlobal("C0MMinBegin","GLOBAL",3)~
END
++ @6 /* That's alright. Might I know your name, though? */ + MINERVA-1
++ @7 /* Quite a singing voice you have. */ + MINERVA-2
++ @8 /* Who are you, gnome, and why should I care? */ + MINERVA-1
++ @9 /* I'm sorry, but I really don't have the time to talk to you. */ + MINERVA-END
...
END
++ @18 /* I'd be happy to have you join me, Minerva. */ + MINERVA-JOIN
+ ~NumInPartyGT(5)~ + @19 /* I've got no more room for new companions, sadly. */ + MINERVA-END
+ ~!NumInPartyGT(5)
NumInPartyGT(1)~ + @20 /* I've got all the company I need at the moment. */ + MINERVA-END
+ ~!NumInPartyGT(1)~ + @21 /* I'd rather travel alone for the time being. */ + MINERVA-END
++ @22 /* No way. Beat it, gnome. */ + MINERVA-END
...
@25 /* Hello there! Is this just a friendly visit, or do you need something from me? I'm itching for an adventure. */
END
++ @18 /* I'd be happy to have you join me, Minerva. */ + MINERVA-JOIN-2
+ ~NumInPartyGT(5)~ + @19 /* I've got no more room for new companions, sadly. */ EXIT
+ ~!NumInPartyGT(5)
NumInPartyGT(1)~ + @20 /* I've got all the company I need at the moment. */ EXIT
+ ~!NumInPartyGT(1)~ + @21 /* I'd rather travel alone for the time being. */ EXIT
...
END
++ @29 /* No, I made a mistake. Stay. */ DO ~JoinParty()~ EXIT
+ ~!AreaCheck("AR1008")
!AreaCheck("AR2114")~ + @30 /* Wait here for now. */ DO ~SetGlobal("KickedOut","LOCALS",1)~ EXIT
+ ~AreaCheck("AR1008")~ + @30 /* Wait here for now. */ DO ~MoveToPoint([368.520]) SmallWait(1) Face(W) SetGlobal("KickedOut","LOCALS",1)~ EXIT
+ ~AreaCheck("AR2114")~ + @30 /* Wait here for now. */ DO ~MoveToPoint([235.211]) SmallWait(1) Face(NE) SetGlobal("KickedOut","LOCALS",1)~ EXIT
+ ~!AreaCheck("AR1200")
!AreaCheck("AR1201")
!AreaCheck("AR1008")
!Global("AR2000_visited","GLOBAL",1)
!Global("AR2100_visited","GLOBAL",1)~ + @31 /* I'll meet you at the Snowdrift Inn. */ DO ~EscapeAreaMove("AR1008",368,520,W) SetGlobal("KickedOut","LOCALS",1)~ EXIT
+ ~OR(2)
AreaCheck("AR1200")
AreaCheck("AR1201")
!Global("AR2000_visited","GLOBAL",1)
!Global("AR2100_visited","GLOBAL",1)~ + @32 /* Go back to Easthaven for now. I'll meet you at the Snowdrift Inn. */ DO ~EscapeAreaMove("AR1008",368,520,W) SetGlobal("KickedOut","LOCALS",1)~ EXIT
...
AreaCheck("AR2115")
AreaCheck("AR2116")
Global("AR2100_visited","GLOBAL",1)~ + @33 /* I'll meet you at the Evening Shade. */ DO ~EscapeAreaMove("AR2114",235,211,NE) SetGlobal("KickedOut","LOCALS",1)~ EXIT
+ ~!Global("Master_Quest","GLOBAL",1)
!Global("Master_Quest","GLOBAL",2)
!Global("Master_Quest","GLOBAL",3)
!Global("Master_Quest","GLOBAL",4)
...
!AreaCheck("AR2115")
!AreaCheck("AR2116")
Global("AR2100_visited","GLOBAL",1)~ + @34 /* Go back to Kuldahar for now. I'll meet you at the Evening Shade. */ DO ~EscapeAreaMove("AR2114",235,211,NE) SetGlobal("KickedOut","LOCALS",1)~ EXIT

CHAIN IF WEIGHT #-1 ~Global("KickedOut","LOCALS",1)~ THEN C0MMinP MINERVA-REJOIN
@35 /* Oh, hello there! Have you come to pick me up? */
END
++ @36 /* Yes, join me. */ DO ~SetGlobal("KickedOut","LOCALS",0) JoinParty()~ EXIT
++ @37 /* No, not yet. */ EXIT
++ @38 /* I'll be back for you later. */ EXIT